...
class Chat:
    ...
    def verificar_mensagem(self,quantidade:int,destinatario:Contato):
        ...
        #         WHERE m.tel_remetente = '{self.telefone_usuario}' 
        #             AND m.tel_destinatario = '{destinatario.telefone}'
        #             OR m.tel_remetente = '{destinatario.telefone}' 
        #             AND m.tel_destinatario = '{self.telefone_usuario}'
        #             ORDER BY m.id_mensagem"""
        
        sql = f"""SELECT nome, mensagem FROM tb_mensagem m 
                INNER JOIN tb_usuario u 
                ON m.tel_remetente = u.tel 
                WHERE m.tel_destinatario = '{self.telefone_usuario}'
                    ORDER BY m.id_mensagem"""
